...
routeServer :: Monad m => Int -> (forall a. m a -> IO a) -> A.RequestParser m B.ResponseBuilder -> IO ()
routeServer port runBase route =
  E.run port (routeApplication runBase route)

routeApplication :: Monad m => (forall a. m a -> IO a) -> A.RequestParser m B.ResponseBuilder -> D.Application
routeApplication runBase route =
  \request responseHandler ->
    runBase (C.route (routerRequest request) route) >>=
    responseHandler . waiResponse . either internalError id
  where
